...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const [busRes, routeRes, driverRes] = await Promise.all([
        supabase.from('buses').select('id, bus_number').ilike('bus_number', q).limit(4),
        supabase.from('routes').select('id, route_name, start_location, end_location').ilike('route_name', q).limit(4),
        supabase.from('profiles').select('id, name').eq('role', 'driver').ilike('name', q).limit(4),
      ])

      const results: SearchResult[] = [
        ...(busRes.data ?? []).map(b => ({
          type: 'bus' as const, id: b.id, label: b.bus_number, sublabel: 'Fleet Bus', path: '/buses',
        })),
        ...(routeRes.data ?? []).map(r => ({
        ...
        ...(driverRes.data ?? []).map(d => ({
          type: 'driver' as const, id: d.id, label: d.name, sublabel: 'Driver', path: '/drivers',
        })),
      ]
  ...
  async function loadNotifications() {
    const { data } = await supabase
      .from('trips')
      .select('id, status, start_time, buses(bus_number), routes(route_name)')
    ...
    if (data) {
      setNotifications(prev => {
        ...
        return data.map(t => ({
          ...
          title: `${(t as any).buses?.bus_number ?? 'Bus'} — ${(t as any).routes?.route_name ?? 'Route'} · ${t.status}`,
  ...
  const typeIcon: Record<string, string> = { bus: '🚌', route: '📍', driver: '👤' }
