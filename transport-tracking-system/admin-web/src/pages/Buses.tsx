...
export default function Buses() {
  ...
  const [form, setForm] = useState({ bus_number: '', capacity: '', driver_id: '' })
  ...
  function openAdd() {
    setEditId(null)
    setForm({ bus_number: '', capacity: '', driver_id: '' })
    setModalOpen(true)
  }

  function openEdit(bus: any) {
    setEditId(bus.id)
    setForm({ bus_number: bus.bus_number, capacity: String(bus.capacity), driver_id: bus.driver_id ?? '' })
    setModalOpen(true)
  }
    ...
    <>
      <div className="page-header">
        <div className="page-header__left">
          ...
          <p>Manage all buses in your transit network</p>
        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Bus' : 'Add New Bus'}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input id="bus_number" placeholder="e.g. KA-01-4521" required value={form.bus_number} onChange={(e) => setForm({ ...form, bus_number: e.target.value })} />
          </div>
          <div className="form-group">
