  ...
  useEffect(() => {
    async function fetchLogs() {
      const [busRes, driverRes, tripRes] = await Promise.all([
        supabase
          .from('vehicles')
          .select('id, vehicle_number, capacity, created_at')
        ...
        supabase
          .from('trips')
          .select('id, status, created_at, vehicles(vehicle_number), routes(route_name)')
          .order('created_at', { ascending: false })
          .limit(100),
      ])
      ...
      for (const b of busRes.data ?? []) {
        entries.push({
          ...
          type: 'bus',
          action: 'Bus Added',
          detail: `Vehicle ${b.vehicle_number} (${b.capacity} seats) was added to the fleet`,
      ...
      for (const t of tripRes.data ?? []) {
        const bus = (t as any).vehicles?.vehicle_number ?? 'Unknown vehicle'
