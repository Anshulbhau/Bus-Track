...
export function useBuses() {
  return useQuery<Vehicle & { profiles: Profile | null }>(() =>
    supabase
      .from('vehicles')
      .select('*, profiles(*)')
...
export function useTrips() {
  return useQuery<Trip & { vehicles: Vehicle | null; routes: Route | null; profiles: Profile | null }>(
    () =>
      supabase
        .from('trips')
        .select('*, vehicles(*), routes(*), profiles(*)')
...
/* ── Route Stops ── */
export function useRouteStopsDetailed(routeId: string) {
  return useQuery<any>(
    () =>
      supabase
        .from('route_stops')
